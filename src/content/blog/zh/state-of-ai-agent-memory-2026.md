...
locale: "zh"
title: "2026 年 AI Agent 记忆现状：基准测试、架构与生产缺口"
...
> - LoCoMo、LongMemEval 和 BEAM 基准测试现已成为比较记忆架构的标准。
>
...
那种认知已经过时了。2026 年，记忆已成为一等架构组件：拥有自己的基准测试套件、独立的研究文献、可量化的方案间性能差距，以及围绕它构建的生态系统。

本报告覆盖了当前的真实状况：基准测试测量什么、各方案如何比较、集成生态长什么样、过去 18 个月技术工作集中在哪，以及哪些问题仍然是真正未解决的。
...
AI agent 记忆研究中最重要的发展，是标准化基准测试的出现——它使得完全不同的记忆架构能在同一评估集上进行比较。目前有三个基准测试定义了测量格局：

1. [**LoCoMo**](https://github.com/snap-research/locomo)：1,540 个问题，横跨四个类别，在多 session 对话数据上测试不同难度级别的记忆召回：单跳（single-hop）、多跳（multi-hop）、开放域（open-domain）和时间记忆召回。在 LoCoMo 之前，记忆质量大多是自报的，或者基于临时任务评估，无法跨实验室复现。
2. [**LongMemEval**](https://github.com/xiaowu0162/longmemeval)：500 个问题，横跨六个类别：单 session 用户召回、单 session 助手召回、单 session 偏好召回、知识更新、时间推理和跨 session 召回。它测试了更广泛的记忆场景，在知识更新和跨 session 任务方面尤其严苛。
3. [**BEAM**](https://github.com/mohammadtavakoli78/BEAM)：该基准测试在 1M 和 10M token 规模上运行，测试记忆系统在上下文量级远超典型基准测试时的表现。BEAM 无法通过简单扩展 context window 来解决，这使它成为与生产规模部署最相关的基准测试。它的十个类别包括偏好遵循、指令遵循、信息抽取、知识更新、跨 session 推理、摘要、时间推理、事件排序、弃权和矛盾解决。
...
这种组合防止了在一个轴上优化而牺牲其他轴。一个准确率很高、但采用 full-context 时每次对话约需 26,000 token 的系统，未必适合生产。一个延迟低但召回率差的系统同样没有实用价值。

### 研究基础

发表在 ECAI 2025 的 Mem0 研究论文（[arXiv:2504.19413](https://arxiv.org/abs/2504.19413)）首次对十种记忆方法进行了广泛的正面比较，包括文献基线、开源工具、RAG、full-context、OpenAI Memory 和 Zep，并在 LoCoMo 基准测试上运行。该论文确立了选择性记忆能达到的基线。Mem0 的新算法显著提升了这个基线。

2026 年 4 月，我们发布了一种新的 token 高效记忆算法，基于单遍层次提取（single-pass hierarchical extraction）和多信号检索（multi-signal retrieval）。以下是改进后的基准测试结果：
...
> 完整评估框架已在 [github.com/mem0ai/memory-benchmarks](https://github.com/mem0ai/memory-benchmarks) 开源。
...
[Graph memory](https://docs.mem0.ai/migration/oss-v2-to-v3#graph-memory-%E2%86%92-entity-linking) 在 2024 年的 AI agent 中基本处于实验阶段。到 2026 年，生产模式已经改变。重要的转变不是"每个 agent 现在都需要图数据库"，而是记忆系统正在超越纯向量相似度。

![向量记忆与图记忆的对比：向量记忆使用嵌入相似度，图记忆映射实体关系和连接](https://framerusercontent.com/images/spAOWCO0vkktuAbZjdTyi1auejU.png)

*图：向量记忆与图记忆的比较*

**向量记忆**检索语义相似的事实。**Graph-style memory（图结构记忆）**通过实体和关系检索事实。两者都有用；单独使用任何一种都不够。

在我们新的[开源算法](https://mem0.ai/research)中，我们用内置实体链接替代了外部图存储支持。在 `add()` 时，Mem0 从每条记忆中提取实体，并将它们存储在名为 `{collection}_entities` 的并行实体集合中。在搜索时，查询中的实体与该集合匹配。这些匹配结果然后在最终组合分数中提升相关记忆的排名。
...
[OpenMemory](https://mem0.ai/openmemory) 是 Mem0 为开发者提供的本地优先记忆层，用于在 AI 工具之间实现持久记忆。它作为 MCP 兼容的记忆服务器运行，支持 [Claude Desktop](https://claude.ai/download)、[Cursor](https://cursor.so/)、[Windsurf](https://codeium.com/windsurf)、VS Code 和其他 MCP 兼容的 agent。记忆存储在本地，配有 dashboard 用于浏览和管理已保存的内容。
...
- **应用级评估：** LoCoMo 上的 91.6 分并不能告诉你系统在医疗或法律工作负载上的表现。基准测试测量的是通用召回。应用级评估对大多数团队来说仍然是手工定制的过程。
...
2026 年的 AI agent 记忆是一个生产工程学科，拥有真实的基准测试、可量化的权衡和不断增长的运维知识体系。

部署记忆的基础设施已经扩展到覆盖 21 个框架、20 个向量存储和三种不同的托管模型——托管云、开源自托管和本地 MCP。剩余的开放问题是真实的，但它们是具体且有边界的，而非根本性的。

- **工程师**现在可以在一个下午内接入持久记忆。[Mem0 Docker 自托管指南](https://mem0.ai/blog/self-host-mem0-docker)使用 Qdrant 作为向量后端，不到 20 分钟就能搭建一个可用的本地 API。如果使用本地开放权重模型，[Hermes agent 教程](https://mem0.ai/blog/how-to-add-memory-to-your-hermes-agent)展示了如何在不依赖云服务的情况下加入跨 session 持久记忆。
- **创始人和架构师**评估记忆层时：token 效率数字是需要压力测试的指标。LoCoMo 上每次检索调用 6,956 token，对比 full-context 每次对话约 26,000 token——单位不同，但在规模化推理账单上仍是需要实测的差异。[基准测试评估框架](https://github.com/mem0ai/memory-benchmarks)已开源——在确定架构之前，先在你自己的工作负载上运行。

| 选项 | 最适合 | 设置时间 |
| --- | --- | --- |
| [Mem0 托管云](https://app.mem0.ai/) | 快速集成，无基础设施开销 | 2 分钟 |
| [自托管 OSS](https://github.com/mem0ai/mem0) | 完全数据控制，规模化成本 | 20 分钟 |
| OpenMemory MCP | 跨开发工具的本地记忆（Claude、Cursor、Windsurf） | 5 分钟 |

- **研究人员**想深入了解评估方法论：我们最新的 [token 高效记忆算法](https://mem0.ai/research)是最佳起点。两个架构变更将语义相似度、BM25 和实体匹配组合为单个融合分数。最大增益来自时间查询（+29.6 分）和多跳推理（+23.1 分）——这两个类别最能反映 agent 如何处理真实用户历史。
...
### 问：哪些基准测试测量 AI agent 记忆质量？

三个基准测试通常定义了这个领域：LoCoMo（1,540 个问题，覆盖单跳、多跳、开放域和时间召回），LongMemEval（500 个问题，包括知识更新和跨 session 召回等类别），和 BEAM（在 1M 和 10M token 规模上跨多个类别评估）。这些基准测试同时测量准确率以及 token 消耗和延迟。

## 来源与参考

- [Mem0：构建具备可扩展长期记忆的生产级 AI Agent（ECAI 2025 论文）](https://arxiv.org/abs/2504.19413)
- [Mem0：Token 高效记忆算法（2026）](https://mem0.ai/blog/mem0-the-token-efficient-memory-algorithm)
- [Mem0 Research](https://mem0.ai/research)
- [评估 LLM Agent 的超长期对话记忆（LoCoMo 论文）](https://arxiv.org/abs/2402.17753)
