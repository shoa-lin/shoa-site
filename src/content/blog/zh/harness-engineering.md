...
![三个同心圆：模型在最核心（被 harness 的最终对象），编码 Agent 的 builder harness 是中间一圈，编码 Agent 的 user harness 是最外层](/assets/blog/harness-engineering/harness-bounded-contexts.png)
...
![变更集成后的持续 feedback sensors 示例：持续 drift 检测（如 /find-dead-code、/code-coverage-quality、dependabot）和持续运行时 feedback（如延迟、错误率、可用性 SLO 触发 Agent 建议，或 /response-quality-sampling、/log-anomalies AI judges）。](/assets/blog/harness-engineering/harness-continuous-feedback-examples.png)
...
![harness 简化概览：水平方向为 guides 和 sensors，垂直方向为三个调节维度——maintainability、architecture fitness 和 behaviour。Behaviour harness 中 spec 作为 feedforward guide，test suite 作为 feedback sensor（混合了 inferential 和 computational），加上人类 icon 表示人工 review 和手动测试作为主要的额外 feedback sensor。](/assets/blog/harness-engineering/harness-types.png)
...
![拓扑示例堆栈：Node 数据仪表盘、JVM CRUD 业务服务、Golang 事件处理器。顶部展示了 data dashboard 的详细结构定义和技术栈，并标注了 "harness template" 包含针对每种拓扑的 guides 和 sensors，可以实例化使用。](/assets/blog/harness-engineering/harness-templates.png)
