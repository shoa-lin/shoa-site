...
> **术语说明**
>
> **Harness**：指 AI Agent 中除模型本身之外的所有组成部分，即 Agent = Model + Harness。在编码 Agent 的上下文中，它既包括构建者内建的系统 prompt、代码检索机制、编排系统等，也包括用户可以自定义的外层控制——如规则文件、Skills、自定义检查脚本等。本文中 Harness 一词保留不译。
>
...
"harness" 已经成为 AI Agent 领域的通用简称，指代模型之外的一切——**Agent = Model + Harness**。但这个定义太宽泛了，值得针对不同的 Agent 类别做进一步收窄。本文聚焦于编码 Agent 这个限界上下文。

在编码 Agent 中，一部分 harness 是由构建者（builder）内建的——系统 prompt、代码检索机制、编排系统等。但编码 Agent 同时也为用户提供了丰富的能力，让我们可以针对自己的用例和系统构建一层**外层 harness**。

![三个同心圆：模型在最核心（被 harness 的最终对象），编码 Agent 的 builder harness 是中间一圈，编码 Agent 的 user harness 是最外层](https://martinfowler.com/harness-engineering/harness-bounded-contexts.png)
...
![概览图：Guides 通过 feedforward 输入编码 Agent（示例包括 [inferential] principles、CfRs、Rules、Ref Docs、How-tos，以及 [computational] Language Servers、CLIs、scripts、codemods）；Sensors 通过 feedback 观察 Agent 的输出并输入其自修正循环（示例包括 [inferential] review agents，以及 [computational] static analysis、logs、browser）。左侧的人类同时操控 Guides 和 Sensors。](https://martinfowler.com/harness-engineering/harness-overview.png)

## Feedforward 与 Feedback

为编码 Agent 构建 harness 的核心思路有两条：

- **Guides（前馈控制）**：预判 Agent 的行为，在它行动**之前**加以引导，从而提高首次生成的成功率。
...
Computational guides 用确定性工具为 Agent 铺路，提高首次生成质量。Computational sensors 便宜且快速，可以配合 Agent 在每次代码变更时运行。Inferential 控制虽然更贵且非确定性，但能提供丰富的语义指导和判断——当与一个强模型（或说适合当前任务的模型）配合时，inferential sensors 对提升信任感尤其有效。
...
![变更生命周期中 feedforward 和 feedback 的示例。Feedforward 包括 LSP、architecture.md、/how-to-test skill、AGENTS.md、MCP server 等，输入 Agent 的初始生成；Feedback sensors 包括 /code-review、npx eslint、semgrep、npm run coverage、npm run dep-cruiser 等；然后是人工 review 作为额外的 feedback；集成后在 pipeline 中重新运行所有 sensors，并增加更昂贵的 /architecture-review skill、/detailed-review skill、mutation testing 等。](https://martinfowler.com/harness-engineering/harness-change-lifecycle-examples.png)
...
![变更集成后的持续 feedback sensors 示例：持续 drift 检测（如 /find-dead-code、/code-coverage-quality、dependabot）和持续运行时 feedback（如延迟、错误率、可用性 SLO 触发 Agent 建议，或 /response-quality-sampling、/log-anomalies AI judges）。](https://martinfowler.com/harness-engineering/harness-continuous-feedback-examples.png)

## 调节类别

Harness 就像一个控制论中的调节器（governor），通过 feedforward 和 feedback 将 codebase 逐步推向期望状态。这个"期望状态"本身有多个维度，每个维度对应一种 harness 类别。做这个区分很有必要——不同类别的 harnessability 和复杂度差异很大，而且用限定词修饰 "harness" 这个泛化的词，能让讨论更精确。
...
![harness 简化概览：水平方向为 guides 和 sensors，垂直方向为三个调节维度——maintainability、architecture fitness 和 behaviour。Behaviour harness 中 spec 作为 feedforward guide，test suite 作为 feedback sensor（混合了 inferential 和 computational），加上人类 icon 表示人工 review 和手动测试作为主要的额外 feedback sensor。](https://martinfowler.com/harness-engineering/harness-types.png)

## Harnessability（可 Harness 性）

并非所有 codebase 都同样适合构建 harness。用强类型语言编写的 codebase 天然拥有类型检查作为 sensor；清晰的模块边界使得架构约束规则成为可能；像 Spring 这样的框架把许多细节抽象掉了，Agent 无需关心，因此间接提升了成功率。**缺乏这些特性，相应的控制手段就无从构建。**
...
![拓扑示例堆栈：Node 数据仪表盘、JVM CRUD 业务服务、Golang 事件处理器。顶部展示了 data dashboard 的详细结构定义和技术栈，并标注了 "harness template" 包含针对每种拓扑的 guides 和 sensors，可以实例化使用。](https://martinfowler.com/harness-engineering/harness-templates.png)

当然，与 service template 一样，一旦团队实例化模板，就会逐渐与上游更新脱节。Harness template 同样面临版本管理和贡献的问题——甚至可能更严重，因为非确定性的 guides 和 sensors 更难测试。

## 人类的角色

作为人类开发者，我们把自己的技能和经验当作一层隐式 harness 带到每个 codebase。我们内化了编码规范和最佳实践，我们亲身体验过复杂性带来的认知负担，我们知道自己的名字挂在每个 commit 上。我们还承载着组织层面的上下文——知道团队在做什么、哪些技术债是业务可以容忍的、在这个特定场景中"好的代码"长什么样。我们小步前进，以人类的节奏工作，这种节奏恰好为经验发挥作用提供了思考空间。
